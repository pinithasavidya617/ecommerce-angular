...
export class ProductServiceService {
  ...
  loadProducts(): void {
    this.apiService.apiGet("/products").subscribe(
      (value: any) => {
  ...
  getFeaturedProducts(): Observable<Product []> {
    return this.apiService.apiGet<Product[]>("/products", {} as HttpParams).pipe(
  ...
  getProductsByCategory(category: string): Observable<Product[]> {
    if(category){
      return this.apiService.apiGet<Product[]>(`/products/category/${category}`).pipe(
        map((response: any) => response.products)
      )
    }else{
      return this.apiService.apiGet<Product[]>("/products", {} as HttpParams).pipe(
